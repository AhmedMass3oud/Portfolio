  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      if (s === 'hero-loop') {
        ...
        // Elapsed-time interpolation — immune to dt drift
        if (!v.paused) v.pause();
        if (reverseStartTimeRef.current === 0) reverseStartTimeRef.current = now;
        const REVERSE_DUR = TRANS_END - HERO_END;
        const elapsed  = (now - reverseStartTimeRef.current) / 1000;
        const progress = Math.min(elapsed / REVERSE_DUR, 1);
        v.currentTime  = TRANS_END - progress * REVERSE_DUR;
        if (progress >= 1) {
          reverseStartTimeRef.current = 0;
    ...
    rafRef.current = requestAnimationFrame(tick);
    return () => { cancelAnimationFrame(rafRef.current); v.pause(); };
  }, []);
        ...
        {isHero ? (
          ...
          <>
            <div className={`shrink-0 pb-6 ${section.id === 'about' ? 'pt-0' : 'pt-20 md:pt-28'} ${section.id === 'contact' ? '' : ''}`}>
              ...
              {section.subtitle && (
                <FadeIn delay={0.2}>
                  <p className={`text-gray-600 font-light leading-tight mt-4 ${section.id === 'contact' ? 'text-lg' : 'text-fluid-subtitle'}`}>{section.subtitle}</p>
                </FadeIn>
              )}
            </div>
      ...
      <div className="relative z-10 flex-1 flex flex-col items-center justify-center px-6 text-center pt-16">
        ...
        >
          ...
          <div className="max-w-lg mx-auto mt-10">
            <p
              className="font-light tracking-tight mb-8 text-white font-sans"
